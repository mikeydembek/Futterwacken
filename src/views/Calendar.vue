    ...
    <div class="details-card">
      ...
      <div v-else class="reminder-list">
        <div
          ...
          :key="rem.id + '-' + (rem.reminderIndex ?? 'm')"
          class="reminder-item"
        >
...
computed: {
  ...
  completedCount() {
    return this.calendarDays.reduce((sum, d) => {
      ...
      return sum + d.reminders.filter(r => r.currentReminder?.completed).length
    }, 0)
  },
  pendingCount() {
...
methods: {
  ...
  getDotClass(reminder) {
    if (reminder.currentReminder?.completed) return 'dot-done'
    const date = new Date(reminder.currentReminder?.date)
    const today = new Date(); today.setHours(0, 0, 0, 0)
    if (date < today) return 'dot-overdue'
    return 'dot-pending'
  },
  getStatusClass(rem) {
    if (rem.currentReminder?.completed) return 'status-done'
    const d = new Date(rem.currentReminder?.date)
    const today = new Date(); today.setHours(0, 0, 0, 0)
    return d < today ? 'status-overdue' : 'status-pending'
  },
  getStatusText(rem) {
    if (rem.currentReminder?.completed) return 'Done'
    const d = new Date(rem.currentReminder?.date)
    const today = new Date(); today.setHours(0, 0, 0, 0)
